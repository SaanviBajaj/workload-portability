...
| | Bootc track (`vmware-bootc`) | **Minimal Alpine (this guide)** |
|---|------------------------------|--------------------------------|
| OS inside the VM | Full CentOS Stream 9 bootc image | Tiny Alpine Linux |
| Disk size | ~1.7 GB per VM | **~768 MB per VM** |
| MTV / OpenShift Virt | Supported | **Not supported** (use the bootc track for MTV) |
| Good when | You want a "real" RHEL-like VM / migration | Smallest demo disks |
...
| **streamOptimized** | VMDK format produced at build time for VMware upload |
