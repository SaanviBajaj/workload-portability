...
Builds two **Fedora 43** VMs from the official **Cloud Base Generic** QCOW2, customizes them with `virt-customize`, repacks each disk to **`disk_mb` (default 1740 ≈ 1.7 GiB)**, uploads streamOptimized VMDKs under **`Workload-Portability/`**, and wires the todo demo so web ↔ db talk and the UI is reachable via the bastion.
...
| **`vmware-fedora-cloud` (this)** | Fedora 43 Cloud Base | **~1.7 GiB** provisioned (see size notes) | **Supported** (`fedora64Guest`) |
...
3. **Repacks** to a UEFI GPT disk of size `disk_mb` (ESP + `/boot` + `/` as ext4) — stock Cloud Base is **5 GiB** virtual with a **~1 GiB `/boot`**, so a plain `virt-resize` cannot shrink enough; tar-repack targets ~1.7 GiB
...
|----------|---------|---------|
| `disk_mb` | `1740` | Provisioned disk target after repack (~1.7 GiB) |
...
**Why ~1.7 GiB:** Cloud Base + Podman + a preloaded todo image does not fit in 768 MiB once btrfs is expanded onto ext4 (plus EFI/`/boot` and MTV free space). 1740 MiB is the practical default to try first.

If the payload still cannot fit, the repack step **fails with measured sizes** and suggests a larger `disk_mb` (up to `disk_mb_max`). It will **not** silently invent another custom rootfs.

**600–768 MB** remains aspirational for this stack and is not the default.

### What we will *not* use (and why)

| Candidate | Verdict |
|-----------|---------|
| **Fedora CoreOS** | Reject. Default image is **~10 GiB** provisioned; OSTree/Ignition is a poor fit for tiny disks and MTV demos. |
| **Fedora Minimal / Everything ISO (Anaconda)** | Reject as primary path. Slow, kickstart-heavy, hard to automate to a predictable small result. |
| **Custom `dnf --installroot` rootfs** | Reject. Fought open-vm-tools, boot metadata, and inspection repeatedly. |
...
- Provisioned disk ≤ **`disk_mb`** (default **1740 MB**, ≤ `disk_mb_max`)

Cold MTV migration of `todo-db` / `todo-web` should get past **ImageConversion** without “unsupported operating system” / inspection failure.
